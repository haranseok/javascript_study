...
function trackUserHandler() {
  ...
  getPosition()
    ...
    .catch(err => {
      console.log(err);
      return 'on we go...';
    }) // 어디든 붙을 수 있다. 그러나 중간에 catch를 추가하면 catch 앞의 모든 then은 건너뛰고 catch 후의 then 블록이 실행된다. 그래서 필요한 경우 특정 부분을 건너뛸 수 있고 한 부분이 실패했다고 전체 체인이 멈추지 않고 동작하게 만들어 준다. 그러나 모든 then을 멈추고 싶으면 catch를 맨 뒤에 작성해주면 강제로 멈출 수 있다.
    .then(data => {
